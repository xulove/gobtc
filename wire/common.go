...
//义了向二进制流读写基础数据类型的的方法，同时也定义了读写可变长度整数值和可变长度字符串的方法;
const (
...
//为了避免序列化或反序列化基础数据类型时频数地分配或者释放内存，binarySerializer提供了一个大小固定的“缓存池”，
//当需要缓存时，向“缓存池”“借”指定大小的的byte slice，使用完毕后“归还”。然而，尽管“缓存池”的大小固定，
//当它分配完毕后，后续的申请并不会被阻塞，而是从内存直接分配，使用完毕后交由gc回收。
...
// readElement reads the next sequence(序列) of bytes from r using little endian
// depending on the concrete（具体的） type of element pointed to.
//通过类型断言(type assertion)解析欲读取字节对应的数据类型，
...
//除了可变长度整数值，还有可变长度字符串和可变长度字节流，
// 它们均是通过在字符串或字节流头部添加一个表示实际长度的可变长度整数值来实现的。
