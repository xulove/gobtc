...
type Tx interface {
	...
	//Metadata(), 通过它可以获得根Bucket，
	// 所有的元数据均归属于Bucket，
	// Bucket及其中的K/V对最终存于leveldb中。
	// 在一个Transaction中，对元数据的操作均是通过Metadata()得到Bucket后，
	// 再在Bucket中进行操作的;
	Metadata() Bucket
	...
	// ******************************************************************
	//Commit()和Rollback，在可写Tx中写元数据或者区块后，均需要通过Commit()来提交修改并关闭Tx，
	// 或者通过Rollback来丢弃修改或关闭只读Tx，作用与BoltDB中的一致
